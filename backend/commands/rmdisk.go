...
func ParseRmdisk(tokens []string) (*RMDISK, error) {
	...
	for _, match := range matches {
		...
		case "-path":
			...
			if !strings.HasSuffix(cmd.path, ".mia") {
				return nil, errors.New("la ruta debe terminar con .mia")
			}
	...
	if cmd.path == "" {
		return nil, errors.New("se requiere el parámetro -path")
	}

	// Ejecuta el comando rmdisk
	err := cmd.Execute()
	if err != nil {
		return nil, fmt.Errorf("error al ejecutar el comando rmdisk: %v", err)
	}
